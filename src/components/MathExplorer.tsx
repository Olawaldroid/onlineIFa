...
import { legNames } from "@/lib/odu/facts";
import { MATH_TOPICS } from "@/lib/content/learn";
...
// Binary explorer slider over all 256 values, plus the recursion-depth and
// math-topic cards. Client-only for the slider's interactivity; the Odù
// lookup runs against the real facts module (src/lib/odu/facts.ts).
export function MathExplorer() {
  ...
  const odu = oduFactBySignature(sig);
  const oduNote = odu
    ? odu.isPrimary
      ? "one of the 16 principal Odù"
      : (() => {
          const legs = legNames(odu);
          return `right leg ${legs.right}, left leg ${legs.left}`;
        })()
    : "";

  return (
    <>
      <div className="mt-12 grid gap-6 lg:grid-cols-[1.2fr_.8fr] lg:items-stretch">
            ...
            <div className="flex flex-wrap gap-2">
              {bits.map((b, i) => (
                <div key={i} className="w-[42px] text-center">
                  ...
                  <div className="mt-[7px] font-mono text-[15px] text-ifa-ink">{b === "1" ? "ǀ" : "ǁ"}</div>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="flex flex-col gap-6">
          <div className="flex-1 rounded-2xl bg-ifa-surface p-[30px] text-ifa-cream">
            ...
            <p className="mb-0 mt-3 text-sm leading-[1.7] text-ifa-cream/80">
              The 16 principal Odù (Méjì) are the figures whose two legs are identical. Pairing any
              right leg with any left leg yields the full corpus:
            </p>
          ...
          <div className="rounded-2xl border border-ifa-border/[0.16] bg-ifa-parchment p-[26px]">
            ...
            <p className="mb-0 mt-2 text-[13.5px] leading-[1.65] text-ifa-ink/70">
              1 cast → 2 outcomes → 4 → 8 … 8 casts → 256. A perfect binary tree of depth 8, walked
              one mark at a time.
            </p>
            <div className="mt-4 flex h-16 items-end gap-[5px]">
              {TREE_BARS.map((tb) => (
                <div key={tb.label} title={tb.label} className="flex-1 rounded-t-[3px] bg-ifa-rust" style={{ height: tb.h, opacity: tb.op }} />
              ))}
            </div>
      ...
      <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {MATH_TOPICS.map((mt) => (
          <div key={mt.tag} className="rounded-[14px] border border-ifa-border/[0.16] bg-ifa-parchment p-[22px] transition-colors hover:border-ifa-rust">
