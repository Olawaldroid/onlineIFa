...
import { IFAGRITHM_NOTES, IFAGRITHM_PEOPLE, IFAGRITHM_VIDEO } from "@/lib/content/ifagrithm";

// Claim-vs-research accordion for the "Ifàgrìthm" field notes.
export function IfagrithmAccordion() {
  ...
  return (
    <>
      <p className="mb-0 mt-5 max-w-[700px] text-base leading-[1.7] text-ifa-cream/75">
        ...
        <a href={IFAGRITHM_VIDEO} target="_blank" rel="noopener noreferrer" className="text-ifa-peri hover:text-ifa-peri">
          Watch the source video ↗
        </a>
      </p>
      <div className="mt-[22px] flex flex-wrap gap-[26px] text-[12.5px] text-ifa-cream/65">
        ...
        {IFAGRITHM_NOTES.map((it, i) => {
          ...
          return (
            ...
            >
              ...
              {isOpen ? (
                <div className="grid gap-4 px-[22px] pb-[22px] pt-1 sm:grid-cols-2">
                  ...
                  <div className="rounded border-l-[3px] border-ifa-peri bg-ifa-peri/[0.06] px-[18px] py-4">
                    ...
                    {it.flag ? (
                      <div className="mt-3 border-t border-dashed border-ifa-border pt-[11px]">
                        <div className="mb-1 font-mono text-[10px] tracking-[0.16em] text-ifa-cream/45">CAVEAT</div>
                        <p className="m-0 text-[12.5px] leading-relaxed text-ifa-cream/65">{it.flag}</p>
                      </div>
                    ) : null}
                    <div className="mt-3 font-mono text-[11px]">
                      <a href={it.href} target="_blank" rel="noopener noreferrer" className="text-ifa-peri hover:text-ifa-peri">
                        {it.srcLabel} ↗
                      </a>
                    </div>
                  </div>
